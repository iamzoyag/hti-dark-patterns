...
async def score_justification(request: Request):
    ...
    try:
        scorer = ChatGoogleGenerativeAI(model="gemini-3.7-flash", temperature=0).with_structured_output(JustificationScore)
...
async def handle_chat(chat_data: ChatMessage):
    ...
    llm = ChatGoogleGenerativeAI(model="gemini-3.7-flash", temperature=0.85)
